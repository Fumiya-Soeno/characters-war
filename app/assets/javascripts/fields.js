    ...
    for(var num = 0;num < 16;num ++){
      chars.forEach(c => {
        if(c.name == team_chars[num]){
          var hash = {"id":dummy_id,"name":c.name,"field_id":364+num*16-(Math.floor(team_chars.length/2)-2)*16,"char_id":c.id,"vitality":c.vitality,"attack":c.attack,"speed":c.speed,"movement_id":c.movement_id,"element_id":c.element_id,"team":0};
        ...
        if(c.name == $(this).attr("data").split("")[num]){
          ...
          var hash = {"id":dummy_id,"name":c.name,"field_id":358+num*16-(Math.floor($(this).attr("data").split("").length/2)-2)*16,"char_id":c.id,"vitality":c.vitality,"attack":c.attack,"speed":c.speed,"movement_id":c.movement_id,"element_id":c.element_id,"team":1};
